 ...
 *
 * The _logging context_ consists of [log fields][LogField] that are attached to every log in a
 * scope. The default [withLoggingContext] function uses a thread-local, which doesn't play well
 * with Kotlin coroutines. This function lets you bridge that gap, by returning a [CoroutineContext]
 * that you can pass to [launch][kotlinx.coroutines.launch]/[async][kotlinx.coroutines.async] to
 * inherit logging context fields in spawned coroutines.
 *
 ...
 *     // for the order in a separate statistics service. But we also want the order ID to be
 *     // included on every log.
 *     //
